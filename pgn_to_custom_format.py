...
def read_single_move(fen_board):
    export_array = np.zeros((12*64), dtype=np.float32)
    ...
    for character in fen_board:
        if character == "/":
            continue
        elif character in figure_to_array_position:
            figure_offset = figure_to_array_position[character]
            export_array[figure_offset + position] = 1.0
            position = position + 1
        else:
            position = position + int(character)
